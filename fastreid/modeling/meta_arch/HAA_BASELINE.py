...
class HAA_BASELINE(nn.Module):
    def __init__(self, cfg):
        ...
        self.heads = build_reid_heads(cfg, 1536,  nn.Identity())    # BNNNeck Heads

        #Adaptive Attention Branch
        self.classifier_black = BlackHead(cfg, 2, 1536, nn.Identity(),flag=True)
        self.prob = BlackHead(cfg, 2, 2, nn.Identity(),flag=True)
    ...
    def forward(self, inputs):
        ...
        #Adaptive Attention Branch
        gf = global_feat      
        black_logits, b1_pool_feat = self.classifier_black(gf,targets)
        w, black_logits = self.prob(black_logits,targets)
