...
def write_naic(indices):
    result = {}
    m, n = indices.shape  # query number and gallery number
    # txt_name = '/home/yhl/data/reid/NAIC/NAICtestset/query_a_list.txt'
    # test_query_dir = '/home/yhl/data/reid/NAIC/NAICtestset/query_a'
    # query_name_list = [osp.basename(t[0]) for t in read_txt(txt_name,test_query_dir) ]

    query_name_list = sorted(os.listdir('/home/yhl/data/NAIC2020/image_A/query'))
    gallery_name_list = sorted(os.listdir('/home/yhl/data/NAIC2020/image_A/gallery'))
    gallery_name_list = np.array(gallery_name_list)

    indices_top200 = indices[:, :200]

    for q_idx in range(m):
        result[query_name_list[q_idx]] = gallery_name_list[indices_top200[q_idx]].tolist()

    with open('submission_a.json', 'w', encoding='utf-8') as f:
        json.dump(result, f)

    print("writing NAIC2020 retrieval results finished")

def submit_result(distmat_np,indices_np,out_dir):

    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    distmat_sort = np.sort(distmat_np)
    m, n = indices_np.shape

    fmt = ['%d'] + ['%d','%.3f']*n

    result = np.zeros((m,2*n+1))
    result[:,0] = np.arange(m)

    for i in range(n):
        result[:,2*i+1] = indices_np[:,i]
        result[:,2*i+2] = distmat_sort[:,i]

    np.savetxt(os.path.join(out_dir, 'result.csv'), result, fmt=fmt, delimiter=",")
