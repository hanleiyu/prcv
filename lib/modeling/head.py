...
class HeadBaseline(nn.Module):
    in_planes = 2048

    def __init__(self, num_classes, last_stride, model_path, neck, neck_feat, model_name, pretrain_choice, cfg):
        super(HeadBaseline, self).__init__()
        self.base = build_backbone(model_name, last_stride)
        if 'regnet' in model_name:
            self.in_planes = self.base.in_planes
    ...
    def forward(self, x, label=None, return_featmap=False):
        ...
        local_feat = nn.AdaptiveAvgPool2d((3,1))(featmap)

        head_feat = local_feat[:, :, 0:1, :]

        # global_feat = global_feat.view(global_feat.shape[0], -1)  # flatten to (bs, 2048)
        global_feat = global_feat.flatten(1)
        head_feat = head_feat.flatten(1)
        ...
        if self.training:
            if self.ID_LOSS_TYPE in ('arcface', 'cosface', 'amsoftmax', 'circle'):
                cls_score = self.classifier(feat, label)
                cls_score_head = self.classifier(head_feat, label)
            else:
                cls_score = self.classifier(feat)
                cls_score_head = self.classifier(head_feat)

            return [cls_score,cls_score_head], [feat,head_feat]  # global_feat  # global feature for triplet loss
        else:
            if self.neck_feat == 'after':
                # print("Test with feature after BN")
                return torch.cat((feat,head_feat),1)
            else:
